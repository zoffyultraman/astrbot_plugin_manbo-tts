...
### 映射文件
插件会自动在缓存目录中创建 `md5_mapping.json` 文件，记录每个MD5哈希对应的原始文本。这让你能够：
- 使用 `/manbo-list` 命令查看所有缓存内容
- 了解每个缓存文件对应的文本内容
- 迁移旧版本缓存文件时，未知文本会标记为 `[unknown]`

映射文件采用JSON格式，键为MD5哈希（不带.wav扩展名），值为对应的文本内容。
